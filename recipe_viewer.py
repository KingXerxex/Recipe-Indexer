...
def open_recipe_logger_window():
    ...
    # --- NEW: Function to handle mouse wheel scrolling ---
    # This function is cross-platform for Windows, macOS, and Linux
    def on_mouse_wheel(event):
        # The '-1 *' is to reverse the direction to feel natural
        my_canvas.yview_scroll(-1 * int(event.delta / 120), "units")

    # --- NEW: Bind the mouse wheel to the canvas ---
    # We use bind_all so it works even when the mouse is over a child widget
    my_canvas.bind_all("<MouseWheel>", on_mouse_wheel)


    # All widgets will now be placed in 'second_frame' instead of 'logger_window'
    ...
    def submit_recipe():
        ...
        try:
            ...
            # --- NEW: Sort the entire sheet by the first column (A-Z) ---
            # This sorts the range A2:V by column 1 (the title)
            ...
            # The window now closes automatically for a smoother workflow
            logger_window.destroy()
            
            # Finally, refresh the list in the main app to show the new sorted order
            refresh_recipe_list()
...
def delete_selected_recipe():
    ...
    try:
        # --- NEW LOGIC TO FIND THE CORRECT ROW ---
        ...
        # Calculate the actual row number in the Google Sheet
        # Listbox index + header offset + 1 (to convert from 0-based to 1-based sheet row)
